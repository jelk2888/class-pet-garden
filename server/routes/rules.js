...
/** 可导入的规则包列表（班宠/班级优/吾师等） */
router.get('/packs', authMiddleware, (req, res) => {
  const packs = listPackMeta().map(p => {
    if (p.id === 'dongguo_default') return { ...p, count: DEFAULT_RULES.length }
    return p
  })
  res.json({ packs })
})
...
/**
 * 导入规则包
 * body: { packId, mode: 'merge'|'replace' }
 * merge=跳过同名；replace=清空后写入该包
 */
router.post('/import-pack', authMiddleware, (req, res) => {
  const { packId, mode = 'merge' } = req.body || {}
  if (!packId) return res.status(400).json({ error: '请选择规则包' })

  let rules = getPackRules(packId)
  if (packId === 'dongguo_default') rules = DEFAULT_RULES
  if (!rules?.length) return res.status(404).json({ error: '规则包不存在' })

  const now = Date.now()
  const insert = db.prepare(`
    INSERT INTO evaluation_rules (id, name, points, category, user_id, created_at, is_custom)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `)
  ...
  const tx = db.transaction(() => {
    ...
    const existing = new Set(
      db.prepare('SELECT name FROM evaluation_rules WHERE user_id = ?').all(req.userId).map(r => r.name)
    )
    for (const rule of rules) {
      if (mode === 'merge' && existing.has(rule.name)) continue
      insert.run(
        uuidv4(),
        rule.name,
        rule.points,
  ...
  const all = db.prepare(`
    SELECT * FROM evaluation_rules WHERE user_id = ?
    ORDER BY CASE WHEN points > 0 THEN 0 ELSE 1 END, points DESC, created_at DESC
  `).all(req.userId)

  res.json({ success: true, added, total: all.length, rules: all })
})
