...
const { students, loadStudents, changePet, batchEvaluate, addEvaluation, undoEvaluation } = useStudents()
...
async function handleUndo() {
  if (!currentClass.value) return
  try {
    const res = await undoEvaluation()
    toast.success(res?.studentName ? `已撤销 ${res.studentName} 的最近评价` : '已撤销最近一条评价')
  } catch {
    toast.error('暂无可撤销记录')
  }
}
    ...
    <div v-if="isLoaded" class="overflow-auto">
      ...
      <div class="mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          @click="handleUndo"
          class="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-xl font-medium text-sm shadow-sm hover:bg-orange-50 hover:border-orange-200 transition-all"
          title="撤销最近一条加分（7天内）"
        >↩️ 撤销加分</button>
